...
from secureguard.permissions import User as SecureUser, Role as SecureRole
...
async def run_chat(
    ...
    # —— 按智能体挂载的知识库检索，把命中内容注入上下文（让回答基于它的库）——
    kb_ids = agent.get("kb_ids") or []
    if kb_ids and getattr(state, "kb_service", None) is not None:
        try:
            svc = state.kb_service
            _kb_caller = SecureUser(id=caller["id"], role=SecureRole(caller["role"]),
                                    dept_id=caller.get("dept_id")) if caller else None
            allhits = svc.search(_kb_caller, message) or []
            hits = [h for h in allhits if (h.get("kb_id") in kb_ids)]
            hits = (hits or [])[:5]
            if hits:
                kb_text = "\n\n".join(
                    f"[来源:{h.get('kb_name') or h.get('kb_id')}] "
                    f"{h.get('content') or h.get('text','')}" for h in hits
                )
                prev = ctx.get("file_context", "")
                ctx["file_context"] = (prev + "\n\n" if prev else "") + \
                    "【知识库检索结果（请优先依据并标注来源）】\n" + kb_text
                ctx["kb_hits"] = hits
                yield mk_frame("context", "kb_retrieve",
                               f"已从「{agent.get('name','智能体')}」的知识库检索 {len(hits)} 条", status="done")
            else:
                yield mk_frame("context", "kb_retrieve",
                               "知识库中未检索到相关内容", status="done")
        except Exception:
            pass
